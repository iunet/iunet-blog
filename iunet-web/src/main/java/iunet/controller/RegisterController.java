...
public class RegisterController {
	...
	public BaseResult register(@RequestBody JSONObject req, HttpServletRequest request) {
		...
		try {
			...
			queryMap.put("state", new BigDecimal(0));
			if (registerService.emailHasRegister(queryMap)) {
				log.error("您的邮箱已经注册，但未激活!");
				return BaseResult.returnErrorMessage("您的邮箱已经注册,但未激活,请查收 邮箱地址:" + email);
			}
			queryMap.put("state", new BigDecimal(1));
			if (registerService.emailHasRegister(queryMap)) {
				log.error("邮箱已经被注册,换个邮箱注册!");
				return BaseResult.returnErrorMessage("邮箱已经被注册,换个邮箱注册!");
			}
			...
			String href = "http://" + WebUtil.getRemoteAddr(request) + ":" + WebUtil.getLocalPort(request) + "/"
					+ webContext + "/register/activation.do?uuid=" + uuid;
			...
			activation.setState(new BigDecimal(1));
