...
public class LoginServiceImpl implements LoginService {
	...
	@Override
	public int updateLoginOutTime(BigDecimal user_id) {
		UmLoginLog loginLog = new UmLoginLog();
		loginLog.setUserId(user_id);
		loginLog.setLogoutTime(DateUtil.Timestamp());
		log.info("updateLoginOutTime user_id：{},Time:{}", JSONObject.toJSON(user_id), DateUtil.nowStr());
		return baseLoginDao.updateLoginOutTime(loginLog);
	}
}
